<script lang="ts">
	...
	const enumList = {
		...
		topic: [
			["工作站布局A", "示教器操作", "执行单元调试", "仓库单元调试", "打磨单元调试", "压装单元调试", "数控单元调试", "HMI模拟A", "SCARA机器人"],
			["工作站布局B", "示教器操作", "涂胶调试", "码垛调试", "多工艺调试", "装配调试", "HMI模拟B", "视频"]
		],
	};
	// todo:end
	export default defineComponent({
		...
		setup() {
					...
					if (type === "工业机器人操作与运维") {
						if (topic !== "工作站布局B") {
							...
							player.load(m4, async (obj: any, resolve: () => void) => {
								...
								const redBoxHelper = new THREE.BoxHelper(j60,
									0xffffff);
								...
								const blueBoxHelper = new THREE.BoxHelper(blueBox,
									0x000000);
			...
			async function itemExit(p: any) {
				...
				if (bigType === "教学演示") {} else if (bigType === "实训练习") {} else if (bigType === "在线考核") {
					//   alert("考核成绩上传成功！");
					let url = "";
					...
					let data: any = {
						"appId": "fac0e74c-f6ec-4e10-b7ad-d6fc95d57177",
						"projectStudyId": "5",
...
<template>
	<div class="main">
		...
		<div class="backbtn" v-if="bigType === '教学演示'" @click="backHome()">
			结束演示
		</div>
		<div class="backbtn" v-if="bigType === '实训练习'" @click="backHome()">
			结束实训
		</div>
		<div class="backbtn" v-if="bigType === '在线考核'" @click="backHome()">
			结束考核
		</div>
		...
		<div class="list_box">
			<div class="tips_box">
				<div class="tips_item" v-for="tip in Tips[topic]" :key="tip"
					@click="runtonum(tip.type, tip.nums[0], tip.name)" :class="{
            tip1: tip.type === 'a',
            tip2: tip.type === 'b',
            tip3: tip.type === 'c',
		...
		<div class="buzhou_title" v-if="!topic.includes('工作站布局')">
			实训步骤总览
		</div>
		<!-- <div class="voice"></div> -->
		...
		<div class="paperPage" v-if="paperShow">
			<div style="position: relative" v-if="topic.includes('工作站布局B')">
				<div class="close" @click="paperShow = !paperShow">+</div>
      ...
      <div class="btn2" @click="reset"></div>
		...
		<div v-if="showControls">
			...
			<teaching-pendant ref="shijiaoqi" v-if="topic === '示教器操作'" @back="itemExit"
				@changeTopicNum="changeTopicNum"></teaching-pendant>
			<palletizing ref="maduo" v-if="topic === '码垛调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</palletizing>
			<process ref="duogongyi" v-if="topic === '多工艺调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</process>
			<bssembling ref="zhuangpei" v-if="topic === '装配调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</bssembling>
			<!-- 集成 -->
			<implement ref="zhixing" v-if="topic === '执行单元调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</implement>
			<warehouse ref="cangku" v-if="topic === '仓库单元调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</warehouse>
			<polish ref="damo" v-if="topic === '打磨单元调试'" @back="itemExit" @changeTopicNum="changeTopicNum"></polish>
			<pressfit ref="yazhuang" v-if="topic === '压装单元调试'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</pressfit>
			<numerical-control ref="shukong" v-if="topic === '数控单元调试'" @back="itemExit"
				@changeTopicNum="changeTopicNum"></numerical-control>
			<scara ref="scara" v-if="topic === 'SCARA机器人'" @back="itemExit" @changeTopicNum="changeTopicNum"></scara>
			<layouta ref="layouta" v-if="topic === '工作站布局A'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</layouta>
			<layoutb ref="layoutb" v-if="topic === '工作站布局B'" @back="itemExit" @changeTopicNum="changeTopicNum">
			</layoutb>
		...
		<!-- todo: 原来的 -->
		<!-- <div v-if="!showControls" class="startb" @click="startTest">
			开始{{ bigType }}
		</div> -->
		<!-- todo: 新增，判断存在参数而且unity调用了方法 -->
		<!-- <div v-if="!showControls" class="startb" @click="startTest">开始{{ bigType }}</div> -->

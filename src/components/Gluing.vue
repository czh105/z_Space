...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // async function setJoints(arr: any): Promise<void> {
    //   let scene = player.scene

    //   let posArr = arr.map((m: number) => {
    //     return parseFloat(m.toFixed(2))
    //   })

    //   const url = "http://121.5.221.47:4000";
    //   let json = await fetch(url + "/robot", {
    //     method: "POST",
    //     // credentials: 'include',
    //     headers: {
    ...
    //     // [0.01, 0, 0, 0, 0, 2.22, -0.01, 0, 0, 0.01, 2.22, 0, -0.23, 31.18, 20.63, 1]
    //     mode: "cors",
    //     body: JSON.stringify({
    //       values: posArr,
    //     }),
    ...
    //   if (res.status !== 8) return
    //   console.log(res)
    //   let resultArr: Array<number> = [Math.PI / 180 * (res.joints[0] - 90), Math.PI / 180 * (res.joints[1] - 103), Math.PI / 180 * (res.joints[2] - 48.12), Math.PI / 180 * (res.joints[3] + 179.13), Math.PI / 180 * (res.joints[4] - 151.29), Math.PI / 180 * (res.joints[5] - 90),]
    //   let j1: any = scene.getObjectByName("j1");
    //   let j2: any = scene.getObjectByName("j2");
    //   let j3: any = scene.getObjectByName("j3");
    //   let j4: any = scene.getObjectByName("j4");
    //   let j5: any = scene.getObjectByName("j5");
    //   let j6: any = scene.getObjectByName("j6");

    //   j1.rotation.z = resultArr[0]
    //   j2.rotation.x = resultArr[1]
    //   j3.rotation.x = resultArr[2]
    //   j4.rotation.y = resultArr[3]
    //   j5.rotation.x = resultArr[4]
    //   j6.rotation.y = resultArr[5]
    ...
    const topicOneImg = computed(() => {
      ...
      switch (topicNum.value) {
        ...
        case 7:
          ...
          break;
        case 8:
          return `background-image: url( "./topic1/t1-4.png"  )`;
          break;
        case 8.5:
        ...
        case 9:
          ...
          break;
        case 10:
          return `background-image: url( "./topic1/t1-5.png"  )`;
          break;
        case 10.5:
          return `background-image: url( "./topic1/pp.png"  )`;
          break;
...
<template>
  <div class="controls_box">
    <div class="helppage" v-if="topicNum === 0 && showHelp">
      ...
      <div
        class="startb"
        @click="topicNum++, (showHelp = false), setLeftContent('操作机器人')"
      >
        开始
      </div>
    </div>
    <div class="left_box" v-if="!showHelp">
      ...
      >
        ...
        <div
          class="t1-1 conBtn"
          v-if="topicNum === 1"
          @click="setLeftContent('涂胶调试实训任务'), topicNext(true)"
        >
          下一步
        </div>
        <div
